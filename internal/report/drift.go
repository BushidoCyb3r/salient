...
<html lang="en"><head><meta charset="utf-8"><title>Defilade drift report</title>
...
<h1>Defilade drift report</h1><p class="meta">{{.FromMeta.CreatedAt}} → {{.ToMeta.CreatedAt}}</p>
...
<h2>New edges to critical nodes</h2>{{if .NewEdgesToTop}}<table><tr><th>Source</th><th>Destination</th><th>Port</th></tr>{{range .NewEdgesToTop}}<tr><td>{{.Src}}</td><td>{{.Dst}}</td><td>{{.Port}}</td></tr>{{end}}</table>{{else}}<p class="empty">None</p>{{end}}
<h2>Vanished critical edges</h2>{{if .VanishedCriticalEdges}}<table><tr><th>Source</th><th>Destination</th><th>Port</th></tr>{{range .VanishedCriticalEdges}}<tr><td>{{.Src}}</td><td>{{.Dst}}</td><td>{{.Port}}</td></tr>{{end}}</table>{{else}}<p class="empty">None</p>{{end}}
