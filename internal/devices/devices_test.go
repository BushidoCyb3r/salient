...
func TestDeviceLayerNormalizes(t *testing.T) {
	cases := map[string]string{
		"router": "router", "L3": "router", "gateway": "router",
		"switch": "switch", "l2": "switch",
		"firewall": "boundary", "Edge": "boundary", "boundary": "boundary",
		"printer": "", "": "",
	}
	for in, want := range cases {
		if got := DeviceLayer(in); got != want {
			t.Errorf("DeviceLayer(%q) = %q, want %q", in, got, want)
		}
	}
}

func TestSetDeviceOwnsMasksAndCreates(t *testing.T) {
	var r Registry
	// creates the device by name, masks host bits, dedupes.
	if err := r.SetDeviceOwns("core-rtr", []string{"10.10.40.5/24", "10.10.40.9/24", "10.10.50.0/24"}); err != nil {
		t.Fatal(err)
	}
	if len(r.Devices) != 1 || r.Devices[0].Name != "core-rtr" {
		t.Fatalf("device not created: %+v", r.Devices)
	}
	got := r.Devices[0].OwnsCIDRs
	want := []string{"10.10.40.0/24", "10.10.50.0/24"}
	if len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("OwnsCIDRs = %v, want %v", got, want)
	}
	// bad CIDR leaves the device unchanged.
	if err := r.SetDeviceOwns("core-rtr", []string{"not-a-cidr"}); err == nil {
		t.Fatal("expected error on bad CIDR")
	}
	if len(r.Devices[0].OwnsCIDRs) != 2 {
		t.Fatalf("owns changed after failed set: %v", r.Devices[0].OwnsCIDRs)
	}
	// empty list clears.
	if err := r.SetDeviceOwns("core-rtr", nil); err != nil {
		t.Fatal(err)
	}
	if len(r.Devices[0].OwnsCIDRs) != 0 {
		t.Fatalf("owns not cleared: %v", r.Devices[0].OwnsCIDRs)
	}
}
